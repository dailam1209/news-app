...
const POKEMON_SERVICE_UUID = '0000fdaa-0000-1000-8000-00805f9b34fb';
...
function useBLE(): BluetoothLowEnergyApi {
  ...
  const requestPermissions = async (cb: VoidCallback) => {
    if (Platform.OS === 'android') {
      ...
      // nhỏ hơn 31 không phải xin quyền connect, scan
      if (apiLevel < 31) {
  ...
  const isDuplicateDevice = (devices: Device[], nextDevice: Device) =>
    ...
    // quét các thiết bị có name thì mới cho vào danh sách(k trùng nhau)
  const scanForPeripherals = () =>
  // quét tất cả các UIID và k phân biệt options
    bleManager.startDeviceScan(null, null,async (error, device) => {
      ...
      (device.name !== null || device.localName !== null)
        // (device.name == "DAILAM" ||
        //   device.localName == "DAILAM")
      ) {
        // const services = await device.services();
        // const characteristics = await services[1].characteristics();
        // console.log("Characteristics:",characteristics);
        // console.log("device.name", device.name, device.localName);
